...
CUSTOM_SENSORS = ['außen','black','white']
USE_CUSTOM_SENSORS = False



db_user = 'pennynet_app'
db_password='#mysqlAPP1'
db_host='151.106.100.101'
db_database='pennynet_logger'
...
def index():
	...
	if (USE_CUSTOM_SENSORS):
		sensorTiles = CUSTOM_SENSORS
	else:
		sensorTiles = getSensorTiles(conn)

	initalData = getInitData(conn, sensorTiles)
	print(initalData)
	sensorTileData
